...
struct SettingsView: View {
    ...
    let positiveRange = Array(stride(from: 0.1, through: 1, by: 0.1))
    let negativeRange = Array(stride(from: -0.1, through: -1, by: -0.1))
        ...
        NavigationStack {
            Form {
                Section(positiveLabel) {
                    ...
                    Stepper(value: $positiveRate, in: 0.1...1, step: 0.1) {
                        Text("Value: \(positiveRate, specifier: "%.1f")")
                    }
                    TextEditor(text: $positiveNotes)
                }
                Section(negativeLabel) {
                    TextField("Label", text: $negativeLabel)
                    Stepper(value: $negativeRate, in: -1...(-0.1), step: 0.1) {
                        Text("Value: \(negativeRate, specifier: "%.1f")")
                    }
